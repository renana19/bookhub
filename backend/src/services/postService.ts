...
export const getPostsByUser = async (userId: number): Promise<post[]> => {
    const [rows] = await pool.query("SELECT * FROM posts WHERE userId = ?", [userId]);
    return rows as post[];
};
...
export const updatePost = async (postId: number, updatedPost: Partial<post>): Promise< post| null> => {
  ...
  `;
    ...
    if ((result as any).affectedRows > 0) {
      return { title: updatedPost.title, 
    content: updatedPost.content  } as post; // Return the updated post object
    }
